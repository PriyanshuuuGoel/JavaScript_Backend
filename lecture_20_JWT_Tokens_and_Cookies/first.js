...
//* Before sending the data , we have to verify the Token 
         ...
         const payload = jwt.verify(req.cookies.Token , "rohit@123");
          console.log(payload); // It return the payload which we send , and if invalid Token then throw error which is handle by catch 
          console.log(req.cookies); // It is showing undefined because it is not parsed , so we have to do parsing here 
          res.send(result);
       }
       catch(error){
           res.send("Error : " + error.message);
